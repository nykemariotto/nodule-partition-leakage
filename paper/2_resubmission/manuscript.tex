...
% !!! TITLE PENDING — four candidates are parked in DECISIONS.md (D59) for the advisor meeting.
% The title below is the ORIGINAL one and is now FACTUALLY WRONG for this study: it names the
% patient-vs-nodule contrast, which this work measures as NULL (D58). Do not submit as-is.
\title{Patient-Level Versus Nodule-Level Data Partitioning in Pulmonary Nodule Classification:
...
and survived re-scoring both arms on identical test rows.
% !!! ABSTRACT ENDING + ARM C — PENDING THE REFRAMING DECISION (researcher's, see DECISIONS D59).
% Two things are stale here and must be settled together with the title:
%  (1) the closing sentence below prescribes PATIENT-level partitioning, but this study measures the
%      patient route as NULL (D58) -- nodule-level grouping captures essentially all of the benefit;
%  (2) the abstract does not mention the THREE-ARM design or the route result at all, which is now
%      the paper's distinctive finding.
Patient-level partitioning yields
...
% !!! STALE — FOURTH INSTANCE OF THE SAME DEFECT (title, abstract ending, conclusion, and this).
% This IS the paper's recommendation, and it rests on the patient route, which the three-arm
% experiment measures as null (D58). Two clauses are specifically contradicted by our own data:
% "it measures, in part, a model's ability to recognise patients it has already seen" (undetected
% here), and "we therefore recommend patient-level partitioning" (nodule-level grouping captures
% essentially all of the benefit). Rewrite as ONE set with the other three -- see DECISIONS D59.
...
% =====================================================================================
\section{Conclusion}
% !!! STALE — SAME DEFECT AS THE TITLE AND THE ABSTRACT ENDING (researcher's reframing, D59).
% This paragraph says the leak is "patient information" and prescribes PATIENT-level partitioning.
% The three-arm experiment measures the patient route as NULL (D58): grouping by NODULE captures
% essentially all of the benefit, and patient grouping adds no detectable further protection in this
% cohort (interval permits up to +0.046 AUC). Rewrite together with the title and abstract; do not
% fix piecemeal, or the three will disagree with each other.
